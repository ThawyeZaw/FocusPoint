...
- Styling: Tailwind CSS v4 plus a large custom CSS design system in `src/index.css`.
- Icons: `lucide-react`.
- Dates and calendar logic: `date-fns`.
- IDs: `uuid`.
- PWA: `vite-plugin-pwa`, configured in `vite.config.js`.
- Persistence: browser `localStorage`, wrapped by `src/data/mockDatabase.js`.
...
The project allows JavaScript under TypeScript checking through `allowJs: true` in `tsconfig.json`.

## App Architecture

`src/main.jsx` is the entrypoint. It imports the app CSS, initializes the mock database with `db.init()`, and mounts React into `#root`.
...
Active route wiring is in `src/App.jsx`.
...
| `/settings` | Redirects to `/courses` | Not a mounted settings screen |
| `*` | Redirects to `/` | Fallback |

`Settings.jsx` and `ResourceLibrary.jsx` exist in `src/components`, but they are not currently mounted by `App.jsx`. They should be treated as available component code, not visible app routes.
...
Template source data comes from `src/data/curriculumData.js`.
...
Because `/settings` redirects to `/courses`, this component is not part of the active navigation surface.
...
`src/data/mockDatabase.js` is the main data layer. It simulates a backend client while storing data in localStorage. The canonical modern structure is `userCourses`; `subjects` and `topics` are compatibility projections derived from `userCourses`.
...
`src/data/curriculumData.js` exports `curriculumTemplateGroups`, flattened `curriculumTemplates`, and `getCurriculumTemplate(templateId)`.
...
`src/engine/priorityEngine.js` drives the Dashboard recommendation logic.
...
The visual system is concentrated in `src/index.css`.
...
PWA configuration is in `vite.config.js`:
...
- There is no backend, auth, sync, or multi-device persistence; all data is local to the browser.
- `Settings.jsx` and `ResourceLibrary.jsx` are not mounted in the active route tree.
...
- `dist/` exists locally but is ignored by git; generated builds should not be treated as source.
